...
function mergeObjects(objects) {
  return objects.reduce((acc, curr) => {
    const tempEntries = Object.entries(curr);
...
function makeWord(lettersObject) {
  ...
  const resArr = Array(Object.values(lettersObject).flat().length);
  arrKeys.forEach((el) => {
    lettersObject[el].forEach((elem) => {
      resArr[elem] = el;
    });
  });
  return resArr.join('');
...
function sellTickets(queue) {
  let resultChange = 0;
  function isSelling(ticketCost) {
    if (ticketCost - 25 <= resultChange) {
      resultChange += 25;
    }
    return resultChange >= ticketCost - 25;
  }
  return queue.every(isSelling);
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    ...
    if (result.has(keySelector(item))) {
      valueArr = [...result.get(keySelector(item)), valueSelector(item)];
    } else {
      valueArr = [valueSelector(item)];
    }
    result.set(keySelector(item), valueArr);
  });
  return result;
}
